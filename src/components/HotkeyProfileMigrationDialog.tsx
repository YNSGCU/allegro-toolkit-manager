...
export default function HotkeyProfileMigrationDialog({ profile, currentEnvironmentId, onClose, onMigrated }: Props) {
  ...
  const targets = useMemo(
    () => registry?.environments.filter((environment) => environment.id !== currentEnvironmentId) || [],
    [currentEnvironmentId, registry],
  );
  ...
  const inspect = async () => {
    ...
    setBusy(true);
    setError(null);
    const result = await window.atm.checkHotkeyProfileCompatibility(profile.id, targetId);
    setBusy(false);
    if (result.success && result.data) {
      ...
      if (target && typeof window.atm.saveCompatibilityRecord === 'function') {
        await window.atm.saveCompatibilityRecord({
          ...
          summary: `快捷键方案“${profile.name}”静态兼容性检查：${result.data.verdict}`,
          details: result.data.findings.map((finding) => `${finding.title}: ${finding.description}`).join('\n'),
        });
      }
    }
    else setError(result.error || '兼容性检查失败');
  };
  ...
  const migrate = async () => {
    ...
    setBusy(true);
    setError(null);
    const result = await window.atm.migrateHotkeyProfile(profile.id, targetId);
    setBusy(false);
    if (!result.success || !result.data) {
      setError(result.error || '迁移失败');
      return;
    }
    onMigrated(result.data.sharedPcbenv ? '目标版本共享同一个 pcbenv，无需复制方案' : '已在目标 Allegro 环境创建方案副本');
    onClose();
  };

  return (
    <div className="modal-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="modal hotkey-migration-dialog" role="dialog" aria-modal="true" aria-labelledby="hotkey-migration-title">
        <div className="modal-header">
          <div><h2 id="hotkey-migration-title">迁移快捷键方案</h2><p>先检查目标 Allegro 版本，再创建独立方案副本。</p></div>
