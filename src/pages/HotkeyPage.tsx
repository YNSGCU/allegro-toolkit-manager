...
const HotkeyPage: React.FC = () => {
  ...
  useEffect(() => { void legacyLoadAll(); }, []);

  const legacyLoadAll = async () => {
    setLoading(true);
    setError(null);
    try {
      // 1. 加载环境
  ...
  const loadAll = useCallback(async (profileIdOverride?: string) => {
    setLoading(true);
    setError(null);
    try {
      const loaded = await loadHotkeyWorkspaceData(profileIdOverride ?? activeProfileId);
  ...
  useEffect(() => {
    ...
    void loadAll(activeProfileId);
  }, [activeProfileId, loadAll]);
  ...
  const handleApplyProfile = useCallback(async () => {
    ...
    try {
      // 生成 Apply Plan
      const planRes = await window.atm.createApplyPlan(envInfo?.envFilePath || '', activeProfileId);
  ...
  // ── 键名归一化（补齐 primaryKey / modifiers / displayKey / caseVariant / layer） ──
  const enrichedBindings = useMemo(
    () => bindings.map((b) => enrichWithPhysicalKey(b)),
    [bindings]
  );
  const enrichedReserved = useMemo(
    () => reservedBindings.map((b) => enrichWithPhysicalKey(b)),
    [reservedBindings]
  );
  ...
  const summaryLine = [
    ...
    envInfo?.envFilePath ? '用户层 env' : 'env 待配置',
  ];
  const promptMessage = [
    `当前方案：${activeProfileName}`,
    plan ? '有待应用变更，可以先检查 Apply Plan。' : '可以继续编辑、排查冲突或直接应用。',
  ].join(' ');
  ...
  return (
    <div className="workspace-page workspace-page-hotkeys">
      <MinimalSurface
        ...
        cards={hotkeySurface.actions.map((action) => ({
          id: action.id,
          title: action.label,
          meta: action.meta,
        }))}
      ...
      {error && (
        <div className="message message-error">
          {error}
          <button className="btn btn-sm" style={{ marginLeft: 12 }} onClick={() => { void loadAll(); }}>重试</button>
        </div>
      )}
      ...
      {/* ── 统计卡片 ── */}
      {false && <div className="grid-3" style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button className="btn btn-sm" onClick={() => { void loadAll(); }} style={{ marginLeft: 'auto' }}>
          刷新
        </button>
      </div>
