...
contextBridge.exposeInMainWorld('atm', {
  ...
  /** 校验工作区跨模块引用一致性（菜单/快捷键命令 ↔ Skill 方案） */
  workspaceCheckRefs: (workspaceId: string) =>
    ipcRenderer.invoke('workspace:check-refs', workspaceId),
  ...
  /** 确认导入工作区方案文件（重新读取校验并创建新工作区；remap 用于换机重绑缺失方案） */
