...
快捷键页只保留“键位 / 冲突”两个任务页签，不再按窗口尺寸缩放键盘或文字。键盘、搜索列表和详情共享一个键位页，导入导出与历史由工具弹窗承载；旧的 overview/editor/import-export 路由重定向到键位页。`.hotkey-workspace-content` 是页面纵向滚动所有者，键盘网格子项必须同时设置 `min-width: 0`，避免固有宽度撑破窄视口。
