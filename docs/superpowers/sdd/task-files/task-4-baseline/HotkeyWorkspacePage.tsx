...
function HotkeyEditorPanel({ sharedState, actions }: HotkeyEditorPanelProps) {
  ...
  return (
    <Placeholder
      title="键位编辑"
      summary="真实编辑面板后续再回接，这里先保留共享容器已经提供的数据和动作。"
    >
...
function HotkeyConflictsPanel({ sharedState, actions }: HotkeyConflictsPanelProps) {
  ...
  return (
    <Placeholder
      title="冲突处理"
      summary="冲突页暂时仍为占位面板，但已经连上真实冲突数据和共享动作。"
    >
...
function HotkeyImportExportPanel({ sharedState, actions }: HotkeyImportExportPanelProps) {
  return (
    <Placeholder
      title="导入导出"
      summary="导入导出仍先保持现状，只把共享容器里的真实动作和状态接进来。"
    >
  ...
  return (
    <div className="hotkey-workspace-page">
      ...
      <Routes>
        ...
        <Route path="overview" element={<HotkeyOverviewPanel state={sharedState} />} />
