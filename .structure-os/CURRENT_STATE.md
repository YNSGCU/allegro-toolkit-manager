...
- App status: five routed pages use the shared workspace UI contract；2026-08-01 完成排版、字号、剩余高度与状态文案收口；写入安全和 IPC 边界不变
- Test status: 35 files / 199 tests pass；固定字号键盘、边缘角标安全区和工作区路由已有回归覆盖
...
- `src/components/{KeyboardVisualizer,ProfileBar,MenuTree,MenuPreviewDialog}.tsx`: fixed typography, truthful state and icon/CSS cleanup
